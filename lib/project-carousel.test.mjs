...
test("waits for movement before capturing a gallery pointer", () => {
  assert.equal(shouldCaptureProjectGalleryPointer(3, 4), false);
  assert.equal(shouldCaptureProjectGalleryPointer(12, 0), true);
});

test("does not capture pointers that start on interactive gallery controls", () => {
  assert.equal(shouldCaptureProjectGalleryPointer(24, 0, true), false);
});

test("treats only side-card controls as interactive gallery controls", () => {
  assert.equal(isProjectGallerySideControl("project-gallery-side-control"), true);
  assert.equal(isProjectGallerySideControl("project-gallery-expand"), false);
});

test("maps visible side cards to previous and next navigation", () => {
  assert.equal(getSideCardStep(-1), -1);
  assert.equal(getSideCardStep(1), 1);
  assert.equal(getSideCardStep(-2), 0);
  assert.equal(getSideCardStep(0), 0);
});
